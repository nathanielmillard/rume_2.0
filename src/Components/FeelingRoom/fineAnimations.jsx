...
export const fineBackgroundAnimation = () => {
  const fineTl = gsap.timeline({ repeat: 25, yoyo: true })
  fineTl.to('.fineRoom', {
    duration: 8,
...
export const floatAnimation = (element, scale) => {
  const floatTl = gsap.timeline({ repeat: 200, yoyo: true })
  floatTl.to(element, {
    duration: 10,
    scale,
    x: '+=random(-400, 400)',
...
export const createFineRoom = () => {
  const bubbles = []
  for (let i = 0; i < 13; i++) {
    const randomNum = Math.random() * (40 - 5) + 5
    let color
    if (randomNum % 2 > 1) {
      color = 'hsla(119, 38%, 72%, .8)'
    } else if (randomNum % 2 > 0.6) {
